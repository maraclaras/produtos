function mostrarDetalhes(nome) {
 // Access detalhesProdutos via the window object
 const produto = window.detalhesProdutos[nome];
 if (!produto) {
 ...
 // Display price
 const priceElement = document.getElementById("detalhes-preco");
 if (typeof produto.preco === "number") {
   priceElement.textContent = `Preço: R$ ${produto.preco.toFixed(2)}`;
 } else {
   priceElement.textContent = `Preço: ${produto.preco}`;
 }

 // Assuming 'mostrarSecao' is defined globally or in produtos.js and accessible
 mostrarSecao("detalhes");
}

// Attach detalhesProdutos to the window object to make it globally accessible
window.detalhesProdutos = {
    "Brigadeiro": {
        nome: "Brigadeiro",
